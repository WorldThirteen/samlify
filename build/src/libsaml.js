...
var xmldsigjs = __importStar(require("xmldsigjs"));
...
var nodeCrypto = __importStar(require("crypto"));
...
var libSaml = function () {
    ...
    */
    ...
    return {
        ...
        */
        registerECDSAAlgorithms: function (sig) {
            // Helper to create ECDSA algorithm class for a specific hash
            var createECDSAAlgorithm = function (hashAlgorithm) {
                return /** @class */ (function () {
                    function ECDSASignatureAlgorithm() {
                    }
                    ECDSASignatureAlgorithm.prototype.getSignature = function (signedInfo, privateKey) {
                        var sign = nodeCrypto.createSign(hashAlgorithm);
                        sign.update(signedInfo);
                        var derSignature = sign.sign({
                            key: privateKey.toString(),
                            format: 'pem',
                            type: 'pkcs8'
                        });
                        // Convert DER signature to IEEE P1363 format for ECDSA
                        var p1363Signature = utility_1.default.derToP1363(derSignature);
                        return p1363Signature.toString('base64');
                    };
                    ECDSASignatureAlgorithm.prototype.verifySignature = function (str, key, signatureValue) {
                        var _a;
                        var verify = nodeCrypto.createVerify(hashAlgorithm);
                        verify.update(str);
                        // Handle both PEM and DER format certificates
                        var certKey = key;
                        if (Buffer.isBuffer(key)) {
                            // Check if it's DER format (binary) - try to convert to PEM
                            var keyString = key.toString('utf8');
                            if (!keyString.includes('-----BEGIN')) {
                                // It's likely DER format - convert to PEM
                                var base64Cert = key.toString('base64');
                        ...
                        var p1363Sig = Buffer.from(signatureValue, 'base64');
                        var derSig = utility_1.default.p1363ToDer(p1363Sig);
                        return verify.verify(certKey, derSig);
                    };
                    ECDSASignatureAlgorithm.prototype.getAlgorithmName = function () {
            ...
            sig.SignatureAlgorithms['http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha1'] = createECDSAAlgorithm('SHA1');
            sig.SignatureAlgorithms['http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha256'] = createECDSAAlgorithm('SHA256');
            sig.SignatureAlgorithms['http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha384'] = createECDSAAlgorithm('SHA384');
            sig.SignatureAlgorithms['http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha512'] = createECDSAAlgorithm('SHA512');
        },
        constructSAMLSignature: function (opts) {
            // Decrypt key first if encrypted, then detect type
            var keyString = Buffer.isBuffer(opts.privateKey)
                ? opts.privateKey.toString('utf8')
                : opts.privateKey;
            var decryptedKeyString = utility_1.default.readPrivateKey(keyString, opts.privateKeyPass);
            var keyType = utility_1.default.detectKeyType(decryptedKeyString);
            // Normalize signature algorithm based on key type (converts RSA→ECDSA if needed)
            var signatureAlgorithm = utility_1.default.normalizeSignatureAlgorithm(opts.signatureAlgorithm, keyType);
            if (keyType === 'EC') {
                // Use xml-crypto with custom EC signer
                return this.constructECXMLSignature(__assign(__assign({}, opts), { signatureAlgorithm: signatureAlgorithm || signatureAlgorithms.ECDSA_SHA256 }));
            }
            else {
        ...
        */
        constructRSAXMLSignature: function (opts) {
        ...
        */
        constructECXMLSignature: function (opts) {
            var rawSamlMessage = opts.rawSamlMessage, referenceTagXPath = opts.referenceTagXPath, privateKey = opts.privateKey, privateKeyPass = opts.privateKeyPass, _a = opts.signatureAlgorithm, signatureAlgorithm = _a === void 0 ? signatureAlgorithms.ECDSA_SHA256 : _a, _b = opts.transformationAlgorithms, transformationAlgorithms = _b === void 0 ? [
                'http://www.w3.org/2000/09/xmldsig#enveloped-signature',
                'http://www.w3.org/2001/10/xml-exc-c14n#',
            ] : _b, signingCert = opts.signingCert, signatureConfig = opts.signatureConfig, _c = opts.isBase64Output, isBase64Output = _c === void 0 ? true : _c, _d = opts.isMessageSigned, isMessageSigned = _d === void 0 ? false : _d;
            // Decrypt the key if encrypted
            var decryptedKey = utility_1.default.readPrivateKey(privateKey, privateKeyPass, true);
            ...
            var ECDSASignatureAlgorithm = /** @class */ (function () {
                function ECDSASignatureAlgorithm() {
                }
                ECDSASignatureAlgorithm.prototype.getSignature = function (signedInfo, privateKey) {
                    var hashAlgNode = hashAlgorithm.replace('-', ''); // Convert SHA-256 to SHA256
                    var sign = nodeCrypto.createSign(hashAlgNode);
                    sign.update(signedInfo);
                    var derSignature = sign.sign({
                        key: privateKey.toString(),
                        format: 'pem',
                ...
                ECDSASignatureAlgorithm.prototype.verifySignature = function (str, key, signatureValue) {
                    ...
                    var hashAlgNode = hashAlgorithm.replace('-', ''); // Convert SHA-256 to SHA256
                    var verify = nodeCrypto.createVerify(hashAlgNode);
                    verify.update(str);
                    // Convert key to PEM if it's DER format
                    var keyPem = key.toString();
                    if (!keyPem.includes('BEGIN CERTIFICATE') && !keyPem.includes('BEGIN PUBLIC KEY')) {
                        // Assume it's DER format, convert to PEM
                        var keyDer = typeof key === 'string' ? Buffer.from(key, 'base64') : key;
                        keyPem = "-----BEGIN CERTIFICATE-----\n".concat((_a = keyDer.toString('base64').match(/.{1,64}/g)) === null || _a === void 0 ? void 0 : _a.join('\n'), "\n-----END CERTIFICATE-----");
                    }
                    // Convert P1363 signature to DER for Node.js crypto.verify()
                    var p1363Sig = Buffer.from(signatureValue, 'base64');
                    var derSig = utility_1.default.p1363ToDer(p1363Sig);
            ...
            sig.SignatureAlgorithms[signatureAlgorithm] = ECDSASignatureAlgorithm;
            // Add assertion sections as reference
            var digestAlgorithm = getDigestMethod(signatureAlgorithm);
            if (referenceTagXPath) {
                sig.addReference({
        ...
        * @param  {SignatureVerifierOptions} opts cert declares the X509 certificate
        ...
        verifySignature: function (xml, opts) {
            ...
            var _loop_1 = function (signatureNode) {
                ...
                this_1.registerECDSAAlgorithms(sig);
        ...
        */
        constructMessageSignature: function (octetString, key, passphrase, isBase64, signingAlgorithm) {
            ...
            var keyString = Buffer.isBuffer(key) ? key.toString('utf8') : key;
            // Detect key type - readPrivateKey now properly handles encrypted PKCS#8 for both RSA and EC
            var decryptedKeyString = utility_1.default.readPrivateKey(keyString, passphrase);
            var keyType = utility_1.default.detectKeyType(decryptedKeyString);
            // Normalize signature algorithm based on key type (converts RSA→ECDSA if needed)
            // For EC keys, default to SHA256 if not specified
            var algorithm = utility_1.default.normalizeSignatureAlgorithm(signingAlgorithm, keyType);
            ...
            if (keyType === 'EC') {
                ...
                var pemKey = decryptedKeyString;
                // Determine hash algorithm from signing algorithm (algorithm is guaranteed to be set for EC)
                var effectiveAlgorithm = algorithm; // Non-null assertion: we set default above for EC keys
                ...
                var decryptedKey = new node_rsa_1.default(decryptedKeyString, // Use the already-decrypted key
                undefined, {
        ...
        */
        verifyMessageSignature: function (metadata, octetString, signature, verifyAlgorithm) {
            return (0, verify_1.verifyMessageSignature)(metadata, octetString, signature, verifyAlgorithm);
        },
        /**
