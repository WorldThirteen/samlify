...
function buildRedirectURL(opts) {
    ...
    if (isSigned) {
        ...
        var keyString = Buffer.isBuffer(entitySetting.privateKey)
        ...
        var decryptedKeyString = utility_1.default.readPrivateKey(keyString, entitySetting.privateKeyPass);
        ...
        var signatureAlgorithm = utility_1.default.normalizeSignatureAlgorithm(entitySetting.requestSignatureAlgorithm, keyType) || entitySetting.requestSignatureAlgorithm; // Fallback to original if no normalization needed
        var sigAlg = pvPair(urlParams.sigAlg, encodeURIComponent(signatureAlgorithm));
        var octetString = samlRequest + relayState + sigAlg;
        return baseUrl
            + pvPair(queryParam, octetString, noParams)
            + pvPair(urlParams.signature, encodeURIComponent(libsaml_1.default.constructMessageSignature(queryParam + '=' + octetString, entitySetting.privateKey, entitySetting.privateKeyPass, undefined, signatureAlgorithm).toString()));
    }
